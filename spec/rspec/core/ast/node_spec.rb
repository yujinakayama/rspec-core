...
module RSpec::Core::AST
  RSpec.describe Node, :skip => (!defined?(RUBY_ENGINE) || RUBY_ENGINE != 'ruby' || RUBY_VERSION < '1.9.2') do
    let(:root_node) do
      Node.new(sexp)
    end
    ...
    describe '#each_ancestor' do
      let(:target_node) do
        root_node.each_node.find { |node| node.type == :arg_paren }
      end
    ...
    describe '#inspect' do
      it 'returns a string including class name and node type' do
        expect(root_node.inspect).to eq('<RSpec::Core::AST::Node program>')
      end
    end
  end
end
